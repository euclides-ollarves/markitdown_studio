...
#### A. En OpenCode (`opencode.json` o `~/.config/opencode/opencode.jsonc`):
```jsonc
{
  ...
  "mcp": {
    "markitdown": {
      "type": "remote",
      "url": "http://<IP_DEL_SERVIDOR>:8000/mcp/sse",
      "enabled": true
    }
  }
}
```
> **Nota para OpenCode**: Si configuras desde la interfaz gráfica de OpenCode y no usas autenticación, **deja la sección `Headers` vacía** (o elimina las filas vacías de headers) para evitar el mensaje de error de cabeceras.

#### B. En Codex (Modo Remoto / Docker):
...
5. *(Headers: déjalo vacío si no requiere token).*

#### C. En Antigravity IDE / Cursor (Modo Remoto):
```json
{
  "mcpServers": {
    "markitdown": {
